...
import (
	"context"
	"fmt"
	ent2 "module-clean/internal/modules/member/driver/persistence/ent"
)

// The MemberFunc type is an interface_adapter to allow the use of ordinary
// function as Member mutator.
type MemberFunc func(context.Context, *ent2.MemberMutation) (ent2.Value, error)

// Mutate calls f(ctx, m).
func (f MemberFunc) Mutate(ctx context.Context, m ent2.Mutation) (ent2.Value, error) {
	if mv, ok := m.(*ent2.MemberMutation); ok {
		return f(ctx, mv)
	}
